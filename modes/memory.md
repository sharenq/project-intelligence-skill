...
3. **標記有效期**（依據以下判斷標準）：
   - `long_term`：架構決策、核心行為規則、跨模組的設計約束。典型來源：技術選型、系統邊界定義、核心抽象設計
   - `medium_term`：目前的實作慣例、模組內部約定。典型來源：coding pattern、錯誤處理策略、測試慣例。可能隨重構改變
   - `short_term`：特定版本的注意事項、臨時修補、已知但尚未修復的問題。典型來源：hotfix、workaround、待清理的技術債
